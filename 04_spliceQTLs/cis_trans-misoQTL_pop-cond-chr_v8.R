...
InverseNormalRankTransform=function(x){qnorm(rank(x,ties='random')/(length(x)+1),mean(x),sd(x))
...
if(PSI_STANDARD){
    ...
    FPKM_Mat$CreateFromMatrix(t(apply(PSI[,wCondPop],1,InverseNormalRankTransform)))
    show(FPKM_Mat)

    Cvrt=SlicedData$new()
    Cvrt$CreateFromMatrix(t(PCs[match(indiv,PCs$IID),3:4]))

    res=Matrix_eQTL_main(GenoNum_Mat,FPKM_Mat,Cvrt,output_file_name=NULL, pvOutputThreshold=thTrans,output_file_name.cis=NULL,pvOutputThreshold.cis=thCis,snpspos=Map[wFreq,c(2,1,4)],genepos=PSI_Pos,pvalue.hist='qqplot',min.pv.by.genesnp=TRUE)
    ...
    RESCIS$R2=RESCIS$statistic^2/(RESCIS$statistic^2+sum(SampleAnnot$cond==cond)-2)	
	RESCIS$type=rep('psiQTL',nrow(RESCIS))
    RESCIS$perm=perm
   	save(RESCIS,file=paste(HOME,'/03_Analysis/Splicing/Papier_Splicing/ReviewNcomm/MatrixEQTL/Perm',perm,'/Cis-sQTL_',pop,'_',cond,'_chr',CHR,'_V8.Rdata',sep=''))
	cat('printed ',HOME,'/03_Analysis/Splicing/Papier_Splicing/ReviewNcomm/MatrixEQTL/Perm',perm,'/Cis-sQTL_',pop,'_',cond,'_chr',CHR,'_V8.Rdata',sep='')
}
...
if(PSI_RESPONSE & cond>1){
    ...
    GenoNum_Mat$ResliceCombined(sliceSize = 5000)
    FPKM_Mat = SlicedData$new()
	FPKM_Mat$CreateFromMatrix(t(apply(PSI[,wCondPop]-PSI[,w1Pop],1,InverseNormalRankTransform)))
		Cvrt=SlicedData$new()
		Cvrt$CreateFromMatrix(t(PCs[match(indiv,PCs$IID),3:4]))
		res=Matrix_eQTL_main(GenoNum_Mat,FPKM_Mat,Cvrt,output_file_name=NULL, pvOutputThreshold=thTrans,output_file_name.cis=NULL,pvOutputThreshold.cis=thCis,snpspos=Map[wFreq,c(2,1,4)],genepos=PSI_Pos,pvalue.hist='qqplot',min.pv.by.genesnp=TRUE)
		res$cis$eqtls$condition=rep(cond,nrow(res$cis$eqtls))
	...
	RESCIS$snps=as.character(RESCIS$snps)
	RESCIS$gene=as.character(RESCIS$gene)
	RESCIS$type=rep('resp-psiQTL',nrow(RESCIS))
    RESCIS$perm=perm
	save(RESCIS,file=paste(HOME,'/03_Analysis/Splicing/Papier_Splicing/ReviewNcomm/MatrixEQTL/Perm',perm,'/Cis-sQTL_',pop,'_',cond,'_chr',CHR,'_response_V8.Rdata',sep=''))
	cat('printed ',HOME,'/03_Analysis/Splicing/Papier_Splicing/ReviewNcomm/MatrixEQTL/Perm',perm,'/Cis-sQTL_',pop,'_',cond,'_chr',CHR,'_response_V8.Rdata',sep='')
	}
#}
q('no')